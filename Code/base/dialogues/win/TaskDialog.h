...
{
public:
    explicit TaskDialog(HICON aIcon, const wchar_t* apTitle, const wchar_t* apMessage, const wchar_t* apMoreContext, const wchar_t* apOptionalDetails);
    ~TaskDialog();

    TaskDialog(const TaskDialog&) = delete;
    TaskDialog& operator=(const TaskDialog&) = delete;

    void SetDefaultButton(int aDefaultButton) { m_defaultButton = aDefaultButton; }
...
private:
    static HRESULT CALLBACK TaskDialogCallbackProc(HWND hwnd, UINT notification, WPARAM w_param, LPARAM l_param, LONG_PTR ref_data);
